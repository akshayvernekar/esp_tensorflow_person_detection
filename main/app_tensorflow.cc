...
void tf_start_inference(void* params)
{
    ...
    //             // The expected input has to be normalised for the values between 0-1
				...
				// free(temp_buffer);
  				...
  				memcpy((interpreter->input(0))->data.uint8, fb->buf, fb->len);
				...
				uint8 max_prob_index = kPersonIndex;

				if(output->data.uint8[kPersonIndex] < output->data.uint8[kNotAPersonIndex])
				{
					max_prob_index = kNotAPersonIndex;
				}

				if(predictionQueue)
				{
					xQueueSend(predictionQueue,(void*)&kCategoryLabels[max_prob_index],0);
				}
			}
		}
